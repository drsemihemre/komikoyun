...
// Ana dünyada portal + uzakta ayrı oyun platformu
// Gerçek "Steal a Brainrot" mekaniklerine yaklaştırılmış versiyon

const MAIN_PORTAL: [number, number, number] = [0, 1.5, 60]
const ZONE_CENTER: [number, number, number] = [400, 150, 400]

// Konveyor: -18 → +18 X ekseninde ilerleyen parça
const CONVEYOR_START = -18
const CONVEYOR_END = 18
const CONVEYOR_TRAVEL_TIME = 22 // saniye
const SPAWN_INTERVAL = 5 // yeni brainrot aralığı

// 4 base slot konumu (platform etrafında)
...
function GameZone() {
  ...
  useFrame((state) => {
    ...
    // Spawn
    if (t - lastSpawnRef.current > SPAWN_INTERVAL) {
      lastSpawnRef.current = t
      const def = randomBrainrot()
      setBelt((prev) => [
        ...prev.slice(-8), // en fazla 9 tane bant üstünde
        { id: `b${Date.now()}_${Math.random()}`, defId: def.id, spawnedAt: t },
      ])
    }

    // Pasif gelir — 1 sn'de bir owned'ların toplam income'u
  ...
  return (
    <group position={[cx, cy, cz]}>
      ...
      {/* Konveyor üstündeki brainrotlar */}
      {mode === 'classic' &&
        belt.map((item) => (
          <BeltBrainrot
            key={item.id}
...
function BeltBrainrot({
  ...
  useFrame((state) => {
    ...
    // Oyuncu yaklaşıp E basınca onBuy
    const player = getPlayerHandle()
    const pp = player?.getPos()
    if (!pp) return
    ...
    // Otomatik al (yakın + yeterli para)
    ...
    <group ref={groupRef} position={[CONVEYOR_START, 0.2, 0]}>
      ...
      <Html position={[0, 3.8, 0]} center distanceFactor={14} zIndexRange={[10, 0]}>
        ...
        >
          ...
          >
            💰 {def.price} {canAfford ? '→ Yaklaş!' : 'yetersiz'}
          </div>
        </div>
      </Html>
    </group>
...
function BaseSlot({
  ...
  useFrame((state) => {
    if (!owned) return
    const t = state.clock.elapsedTime
    // Yakına gelince auto-lock eğer kilitli değilse (E zahmetsiz)
    ...
    if (Math.hypot(dx, dz) < 1.5 && Math.abs(pp.y - position[1]) < 3) {
      // Oyuncu kendi slotu'nun üzerinde + para yeter → kilitle
